...
module.exports = function(fileContent) {
    ...
    fileContent = replaceSrc(fileContent, query.exclude);
    fileContent = replaceInline(fileContent, query.exclude);
...
function replaceInline(fileContent, exclude) {
    fileContent = fileContent.replace(/\\?["'](\S*?\.(jpg|jpeg|png|gif|svg|webp))\?_inline.*?["']/ig, function(match, str){
    	var imgUrl = str;
        if(/^(http(s?):)?\/\//.test(imgUrl)) return str; // 绝对路径的图片不处理
        if(exclude && imgUrl.indexOf(exclude) != -1) return str; // 不处理被排除的
        return "\"+JSON.stringify(require("+JSON.stringify(imgUrl)+"))+\"";
    });
    return fileContent;
}
